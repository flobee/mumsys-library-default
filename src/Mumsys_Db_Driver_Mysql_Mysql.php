...
/**
 * Mysql Driver for database purpose
 *
 * @category Mumsys
 * @package Mumsys_Library
 * @subpackage Mumsys_Db
 */
class Mumsys_Db_Driver_Mysql_Mysql
    ...
    implements Mumsys_Db_Driver_Interface, Mumsys_Db_Driver_Query_Interface
{
     ...
     *
     * @return resource|false Retruns the database resource on succsess or throws exception
    ...
    public function connect()
    {
        ...
        // resource mysql_connect([server [,username [,password [,bool new_link [,int client_flags]]]]] )
        // client_flags
        ...
        if ( $this->_dbName ) {
            // small hack to use selectDB() on first connection
            $dbName = $this->_dbName;
            $this->_dbName = '--tmp-db-name';
     ...
     *
     * @return bool Returns the result of mysql_close() function. True = success
     */
    public function close()
    {
        $x = true;
        if ($this->_dbc) {
            $x = @mysql_close($this->_dbc);
        }
        $this->_isConnected = false;
        $this->_dbc = null;
        return $x;
    ...
    {
        if ( ($result = mysql_set_charset($charset, $this->_dbc) ) == false ) {
    ...
    {
        if ( ($result = @mysql_client_encoding($this->_dbc) ) == false ) {
     ...
     * Select a database.
     *
     * old: On failure the connection will be closed
     * new: Connection stays-
     *
     * @param string $db Name of the db
     * @return boolean Returns true on success or throws exception
    ...
    public function selectDB($dbName)
    {
        ...
        if ( mysql_select_db($dbName, $this->_dbc) === false ) {
            $this->_dbName = $dbName;
     ...
     *
     * @see http://de2.php.net/manual/en/function.mysql-stat.php
     *
    ...
    {
        return mysql_get_server_info($this->_dbc);
    }
     ...
     * General info by php.net: For SELECT, SHOW, DESCRIBE, EXPLAIN and other
     * statements returning resultset, mysql_query() returns a
    ...
    public function query($sql=false, $unbuffered=false)
    {
        ...
        if ( $unbuffered ) {
            $result = mysql_unbuffered_query($this->_sql, $this->_dbc);
        } else {
            $result = mysql_query($this->_sql, $this->_dbc);
        }
        ...
        if ( ($error = $this->sqlError()) ) {
            $this->_setError($error); // wer wann wie was wo
            return false;
        }

        $oRes = new Mumsys_Db_Driver_Mysql_Mysql_Result($this, $result);
        return $oRes;
    }
    ...
    {
        return (int)@mysql_errno($this->_dbc);
    }
     ...
     * SELECT *
        ...
        WHERE `TABLE_SCHEMA` LIKE 'cachedb'
        AND `TABLE_NAME` LIKE 'feed_content'
     *  or
     * SELECT *  FROM `KEY_COLUMN_USAGE` WHERE `TABLE_SCHEMA` LIKE 'mshop'
       ORDER BY  `TABLE_NAME`, `ORDINAL_POSITION`, `CONSTRAINT_NAME`
     */
    ...
    public function showColumns($table='', $field=null)
    {
        ...
        if ( isset($field) ) {
            ...
            $oRes = $this->query(
                'SHOW COLUMNS FROM ' . $this->_dbName . '.' . $table, $this->_dbc
            );
        }
    ...
    /**
     * Toggle IDs eg. of a mapping/link table
     *
     * Note: columnes should NOT be unsign or this method will fail
     *
     * @todo API change: return true on success!
     *
     * toggleIDs is good to use if you want to change ... mapping/link tables.
     * eg. changing category ids from a nested set
     * @example class.the3DTree.php
     *
     * @param string $db Name of the database
     * @param string $table Table to use for the changes
     * @param string $col Columne of the ID to toggle
     * @param string $idFrom ID where older values can be found
     * @param string $idTo ID to the new value of the $id_from
     * @param string $whereclause Conditions to fetch the id's which should be
     * toggled
     *
     * @return bool return false on success of true on failure
     */
    public function toggleIDs($db=false, $table=false, $col=false,
        $idFrom=null, $idTo=null, $whereclause='')
    {
        try {
            /** @Todo: needs to be tested/ example needed! */
            if ( $whereclause ) {
                $whereclause = ' AND ' . $whereclause;
            }

            if ( $db && $table && $col && isset($idFrom) && isset($idTo) ) {
                $this->selectDB($db);
                ...
                $sql = 'UPDATE `' . $db . '`.`' . $table . '` SET `'
                    . $col . '` = ' . $idFrom . ' WHERE `'
                    . $col . '` = ' . $idTo . ' ' . $whereclause . '';
                $this->query($sql);

                $sql = 'UPDATE `' . $db . '`.`' . $table . '` SET `'
                    . $col . '` = ' . $idTo . ' WHERE `' . $col . '` = -1 '
                    . $whereclause . '';
                $this->query($sql);
        ...
        $this->_setError('Toggle IDs failt');
...
//            }
     ...
     *
     * @param array $limit array containing the limit (array key 0), the offset
     * (array key 1)
    ...
    public function escape($string='')
    {
        ...
        return mysql_real_escape_string($string, $this->_dbc);
    }
